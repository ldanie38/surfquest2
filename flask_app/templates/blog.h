<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Surf Blog</title>
  <link rel="stylesheet" href="/static/css/blog.css" />
  <link rel="stylesheet" href="https://use.typekit.net/gqv8jnv.css"> <!-- Adobe font-->
        <!-- Favicon for most browsers -->
        <link rel="icon" type="image/png" sizes="32x32" href="static/img/favicon-32x32.png">
        <link rel="icon" type="image/png" sizes="16x16" href="static/img/favicon-16x16.png">
        <link rel="shortcut icon" href="/favicon.ico">
        <!-- For Apple devices -->
        <link rel="apple-touch-icon" sizes="180x180" href="/apple-touch-icon.png">
        <!-- Theme color for Chrome, Firefox OS and Opera -->
        <meta name="theme-color" content="#ffffff">
</head>
<body>
  <div class='head'>
    <header>
      <h1>SurfQuest Blog</h1>
      {% if username %}
        <p class='welcome'>Welcome, {{ username|title }}!</p>
      {% else %}
        <p>Welcome to the SurfQuest Blog!</p>
      {% endif %}
    </header>
  </div>

  <div class='container'>

    <div class='img'>
      <img id="surfImage" src="static/img/drone_road_waves.jpg" alt="A surfer riding a wave" title="Epic Surf!">
    </div>

    <div class='total_post'>

      <!-- Blog Post Form (visible only when a user is logged in) -->
      <div class='blog_post'>
        {% if username %}
          <section id="create-post">
            <h2>Share Your Story</h2>
            <form action="/blog/create" method="POST">
              <label for="title">Title:</label>
              <input type="text" id="title" name="title" required>
              
              <label for="content">Content:</label>
              <textarea id="content" name="content" rows="5" required maxlength="5000"></textarea>
              
              <button type="submit">Publish</button>
            </form>
          </section>
          <hr>
        {% endif %}
      </div>

      <div class="post_content">
        <section id="blog-posts">
          {% for post in posts %}
              <article>
                  <h2>{{ post.title }}</h2>
                  <p>{{ post.content }}</p>
                  <p><strong>By:</strong> {{ post.author_username }} | {{ post.created_at.strftime('%Y-%m-%d') }}</p>
      
  <!-- Like Button -->
  <p>
    <button class="like-button" data-post-id="{{ post.id }}">
        ❤️ Like (<span class="like-count">{{ post.likes }}</span>)
    </button>
  </p>

  <!-- Comment Button -->
  <p>
    <button class="comment-button" data-post-id="{{ post.id }}">
        💬 Comment
    </button>
  </p>

      
                  <!-- Display Comments Under Each Post -->
                  <div class="comments-section">
                      {% for comment in post_comments[post.id] %}
                          <p><strong>{{ comment.user_id }}</strong>: {{ comment.content }}</p>
                      {% endfor %}
                  </div>
      
                  <!-- Comment Form for Each Post -->
                  <div id="comment-form-{{ post.id }}" class="comment-form" style="display: none;">
                      <form action="/comment/create" method="POST">
                          <input type="hidden" name="post_id" value="{{ post.id }}">
                          <label for="comment-content">Leave a comment:</label>
                          <textarea id="comment-content" name="content" rows="3" placeholder="Write your comment..." required></textarea>
                          <button type="submit">Post Comment</button>
                      </form>
                  </div>
              </article>
              <hr>
          {% endfor %}
      </section>
      

     
      </div>
    </div>
  </div>

  <div class='end'>
    <a href="/home">Back to Home</a> | 
    <a href="/logout">Logout</a>
  </div>

  <footer>
    <p>Catch the latest waves 🌊</p>
  </footer>
  <script>
    document.addEventListener("DOMContentLoaded", function() {
      const likeButtons = document.querySelectorAll('.like-button');
  
      likeButtons.forEach(function(button) {
        button.addEventListener('click', function(e) {
          e.preventDefault();
          
          const postId = this.dataset.postId;
          const buttonElem = this;
  
          fetch('/like', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
            },
            body: JSON.stringify({ post_id: postId })
          })
          .then(response => response.json())
          .then(data => {
            if (data.success) {
              // Like was successful—update the like count and disable the button.
              const likeCountSpan = buttonElem.querySelector('.like-count');
              if (likeCountSpan) {
                likeCountSpan.textContent = data.likes;
                // Optionally disable button to prevent further clicks.
                buttonElem.disabled = true;
              }
            } else {
              // If not successful, maybe it was already liked. You can show a message.
              console.log(data.message);
              // Optionally disable the button if the post was already liked
              if(data.liked){
                buttonElem.disabled = true;
              }
            }
          })
          .catch(error => console.error('Error:', error));
        });
      });
    });

/* img rotation*/
document.addEventListener("DOMContentLoaded", function() {
  const images = [
      "static/img/drone_road_waves.jpg",
      "static/img/surfer_on_wave.jpg",
      "static/img/index.jpg",
      "static/img/bad_ass.jpg"
  ];

  let index = 0;
  function changeImage() {
      const imgElement = document.getElementById("surfImage");
      if (imgElement) {
          index = (index + 1) % images.length;
          imgElement.src = images[index];
      }
  }

  setInterval(changeImage, 5000); // Change every 5 seconds
});

/*comment section*/
document.addEventListener("DOMContentLoaded", function() {
  document.querySelectorAll(".comment-button").forEach(button => {
      button.addEventListener("click", function() {
          const postId = this.dataset.postId;
          const commentForm = document.getElementById(`comment-form-${postId}`);

          if (commentForm.style.display === "none") {
              commentForm.style.display = "block"; // Show form
          } else {
              commentForm.style.display = "none"; // Hide form
          }
      });
  });
});




  
  </script>


  



  
</body>
</html>


@classmethod
def get_by_post(cls, data):
    query = "SELECT * FROM comments WHERE blog_post_id = %(blog_post_id)s ORDER BY created_at ASC;"
    results = connectToMySQL('project').query_db(query, data)

    if not results:  
        return []  # Ensures an empty list instead of None
    
    comments = [cls(row) for row in results]  # List comprehension for cleaner code

    return comments
